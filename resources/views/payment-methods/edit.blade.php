<x-app-layout>
    ...
    <div class="max-w-md mx-auto">
        <div class="bg-white p-6 sm:p-8 rounded-2xl border border-slate-200/80 shadow-sm">
            <h2 class="text-xl font-bold text-slate-900 mb-6 pb-4 border-b border-slate-100">Edit Payment Method</h2>

            <form method="POST" action="{{ route('payment-methods.update', $paymentMethod->id) }}" class="space-y-4">
                @csrf
                @method('PUT')

                <div>
                    <label class="block text-sm font-bold text-slate-700 mb-1.5">Payment Method Name <span class="text-rose-500">*</span></label>
                    <input type="text" name="name" value="{{ old('name', $paymentMethod->name) }}" required class="w-full rounded-xl border-slate-200 text-slate-900 focus:border-indigo-500 focus:ring-indigo-500">
