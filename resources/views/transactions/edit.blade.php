    ...
    <div class="max-w-2xl mx-auto">
        <div class="bg-white p-6 sm:p-8 rounded-2xl border border-slate-200/80 shadow-sm">
            <div class="mb-6 pb-4 border-b border-slate-100 flex items-center justify-between">
                <div>
                    <h2 class="text-xl font-bold text-slate-900">Edit Transaction</h2>
                    <p class="text-sm text-slate-500">Update transaction details.</p>
                </div>
                <a href="{{ route('transactions.index') }}" class="text-sm font-semibold text-slate-600 hover:text-slate-900">Cancel</a>
            </div>
            ...
            <form method="POST" action="{{ route('transactions.update', $transaction->id) }}" class="space-y-5">
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="number" step="0.01" min="0.01" name="amount" value="{{ old('amount', $transaction->amount) }}" required class="pl-8 w-full rounded-xl border-slate-200 text-slate-900 font-semibold focus:border-indigo-500 focus:ring-indigo-500">
                    </div>
                ...
                <div>
                    ...
                    <input type="text" name="category" value="{{ old('category', $transaction->category) }}" required class="w-full rounded-xl border-slate-200 text-slate-900 focus:border-indigo-500 focus:ring-indigo-500">
                ...
                <div>
                    ...
                    <input type="text" name="description" value="{{ old('description', $transaction->description) }}" class="w-full rounded-xl border-slate-200 text-slate-900 focus:border-indigo-500 focus:ring-indigo-500">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="date" name="date" value="{{ old('date', $transaction->date ? $transaction->date->format('Y-m-d') : '') }}" required class="w-full rounded-xl border-slate-200 text-slate-900 focus:border-indigo-500 focus:ring-indigo-500">
                    ...
                    <div>
                        ...
                        <select name="payment_method" class="w-full rounded-xl border-slate-200 text-slate-900 focus:border-indigo-500 focus:ring-indigo-500">
                ...
                <div>
                    ...
                    <textarea name="note" rows="3" class="w-full rounded-xl border-slate-200 text-slate-900 focus:border-indigo-500 focus:ring-indigo-500">{{ old('note', $transaction->note) }}</textarea>
                    @error('note') <p class="text-xs text-rose-500 mt-1 font-medium">{{ $message }}</p> @enderror
                </div>

                <div class="pt-4 flex items-center justify-end space-x-3">
                    <a href="{{ route('transactions.index') }}" class="px-5 py-2.5 rounded-xl border border-slate-200 text-slate-600 hover:bg-slate-50 font-semibold text-sm">Cancel</a>
