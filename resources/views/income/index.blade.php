<x-app-layout>
    ...
    <div class="space-y-6">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                <h2 class="text-xl font-bold text-slate-900">Income Transactions</h2>
                <p class="text-sm text-slate-500">Track and manage all your revenue streams.</p>
            </div>
            <a href="{{ route('income.create') }}" class="inline-flex items-center px-4 py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-semibold text-sm shadow-md shadow-emerald-600/20 transition-all self-start sm:self-auto">
        ...
        <div class="bg-white p-5 rounded-2xl border border-slate-200/80 shadow-sm">
            <form method="GET" action="{{ route('income.index') }}" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
                <div>
                ...
                <div class="flex items-end gap-2">
                    ...
                    <a href="{{ route('income.index') }}" class="px-3 py-2.5 bg-slate-100 hover:bg-slate-200 text-slate-600 rounded-xl text-sm font-semibold transition-colors">
                        Reset
                    </a>
                </div>
            </form>
        ...
        <div class="bg-white rounded-2xl border border-slate-200/80 shadow-sm overflow-hidden">
            @if($incomes->isEmpty())
                <div class="p-12 text-center">
                    ...
                    <h4 class="text-base font-bold text-slate-800">No income records found</h4>
                    <p class="text-sm text-slate-500 max-w-sm mx-auto mt-1">Start recording your earnings to keep your financial records up to date.</p>
                    <a href="{{ route('income.create') }}" class="mt-5 inline-block px-4 py-2.5 rounded-xl bg-emerald-600 text-white font-semibold text-sm hover:bg-emerald-700 shadow-md">Add Income</a>
                </div>
            @else
                            ...
                            @foreach($incomes as $income)
                                <tr class="hover:bg-slate-50/80 transition-colors">
                                    <td class="py-4 px-6 text-slate-600 whitespace-nowrap">
                                        {{ $income->date ? $income->date->format('M d, Y') : '-' }}
                                    </td>
                                    <td class="py-4 px-6 whitespace-nowrap">
                                        <span class="px-2.5 py-1 rounded-lg text-xs font-medium bg-emerald-50 text-emerald-700 border border-emerald-100">
                                            {{ $income->category }}
                                        </span>
                                    </td>
                                    <td class="py-4 px-6">
                                        <div class="font-semibold text-slate-900">{{ $income->description ?: 'No description' }}</div>
                                        @if($income->note)
                                            <div class="text-xs text-slate-400 truncate max-w-xs">{{ $income->note }}</div>
                                        @endif
                                    </td>
                                    <td class="py-4 px-6 whitespace-nowrap text-slate-600">
                                        {{ $income->payment_method ?: '-' }}
                                    </td>
                                    <td class="py-4 px-6 text-right font-bold text-emerald-600 whitespace-nowrap">
                                        +৳ {{ number_format($income->amount, 2) }}
                                    </td>
                                    <td class="py-4 px-6 text-center whitespace-nowrap">
                                        <div class="flex items-center justify-center space-x-2">
                                            <a href="{{ route('income.show', $income->id) }}" class="p-1.5 text-slate-400 hover:text-indigo-600 rounded-lg hover:bg-indigo-50 transition-colors">
                                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path></svg>
                                            </a>
                                            <a href="{{ route('income.edit', $income->id) }}" class="p-1.5 text-slate-400 hover:text-blue-600 rounded-lg hover:bg-blue-50 transition-colors">
                                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                                            </a>
                                            <form method="POST" action="{{ route('income.destroy', $income->id) }}" onsubmit="return confirm('Are you sure you want to delete this income entry?')">
                                                @csrf
                                                @method('DELETE')
